(function(){'use strict';
let PRECO=20,TOTAL=100,VENDAS_ABERTAS=true,configRifa={},configAfiliados={ativo:true,valorPorNumero:0.20,minimoSaque:10};let selecionados=[],ultimosNumeros=[],reservaAtualId='',mapaNumeros={},pagamentos={};
...
async function reservar(e){
  ...
  const refCodigo=(new URLSearchParams(location.search).get('ref')||localStorage.getItem('rifaIndicadorCodigo')||'').trim().toUpperCase();
  let indicadorCodigo='',indicadorCpf='';
  if(refCodigo){try{const ind=await db.ref('rifa/codigosAfiliados/'+refCodigo).once('value');indicadorCpf=String(ind.val()||'');if(indicadorCpf&&indicadorCpf!==cpf){indicadorCodigo=refCodigo;localStorage.setItem('rifaIndicadorCodigo',refCodigo);}}catch(_){}}
  const r={reservaId:rid,ownerUid:u.uid,nome,telefone,cpf,email,numeros:nums,valor:nums.length*PRECO,status:'processando',criadoEm:agora,indicadorCodigo,indicadorCpf};
...
function aplicarConfigRifa(c){
  ...
  document.title=(configRifa.nomeRifa||'Rifa Online')+' V15.9';
...
async function init(){ if('serviceWorker' in navigator)registrarServiceWorker();$('formRifa').onsubmit=reservar;$('btnInformarPagamento').onclick=informarPagamento;$('btnRepetirNumeros').onclick=repetir;$('btnCopiarPix').onclick=copiarPix;$('btnAtualizar').onclick=()=>location.reload();if($('btnMinhasReservas'))$('btnMinhasReservas').onclick=minhasReservas;$('btnBuscarCpf').onclick=buscarReservasCpf;$('btnConsultarAfiliado').onclick=consultarAfiliado;$('btnSalvarAfiliado').onclick=salvarAfiliado;$('btnCopiarLinkAfiliado').onclick=copiarLinkAfiliado;$('btnNovaReserva').onclick=()=>{$('sucessoBox').className='card sucesso oculto';reservaAtualId='';scrollTo(0,0);};$('telefone').oninput=e=>{let v=digs(e.target.value).slice(0,11);if(v.length>6)v='('+v.slice(0,2)+') '+v.slice(2,7)+'-'+v.slice(7);else if(v.length>2)v='('+v.slice(0,2)+') '+v.slice(2);e.target.value=v;};$('cpf').oninput=e=>{let v=digs(e.target.value).slice(0,11);v=v.replace(/(\d{3})(\d)/,'$1.$2').replace(/(\d{3})(\d)/,'$1.$2').replace(/(\d{3})(\d{1,2})$/,'$1-$2');e.target.value=v;};$('afCpf').oninput=e=>formatarCpfInput(e.target);$('consultaCpf').oninput=e=>{let v=digs(e.target.value).slice(0,11);v=v.replace(/(\d{3})(\d)/,'$1.$2').replace(/(\d{3})(\d)/,'$1.$2').replace(/(\d{3})(\d{1,2})$/,'$1-$2');e.target.value=v;};$('consultaCpf').onkeydown=e=>{if(e.key==='Enter'){e.preventDefault();buscarReservasCpf();}};try{await auth.setPersistence(firebase.auth.Auth.Persistence.LOCAL);if(!auth.currentUser)await auth.signInAnonymously();await registrarVisualizacao();await recuperarReservaPendente();$('mensagemGrade').className='aviso oculto';db.ref('rifa/configPublica/rifa').on('value',s=>aplicarConfigRifa(s.val()||{}));db.ref('rifa/numeros').on('value',s=>{mapaNumeros=s.val()||{};selecionados=selecionados.filter(n=>!mapaNumeros[n]||mapaNumeros[n].status==='livre');desenhar();resumo();});db.ref('rifa/configPublica/pagamentos').on('value',s=>pagamentos=s.val()||{});db.ref('rifa/configPublica/afiliados').on('value',s=>configAfiliados=Object.assign({ativo:true,valorPorNumero:0.20,minimoSaque:10},s.val()||{}));db.ref('rifa/estatisticasPublicas').on('value',s=>{const e=s.val()||{};if($('pubParticipantes'))$('pubParticipantes').textContent=String(Number(e.participantes||0));if($('pubArrecadado'))$('pubArrecadado').textContent=moeda(Number(e.arrecadado||0));});await minhasReservas();}catch(e){$('mensagemGrade').textContent='Erro ao conectar: '+e.message;$('mensagemGrade').className='aviso';}}
